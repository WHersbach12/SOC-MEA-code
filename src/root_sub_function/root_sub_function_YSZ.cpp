...
RootSubFunctionYSZ::RootSubFunctionYSZ( const int _rank, const std::vector<double>& _x_center, const std::vector<double>& _x_face, const double _temperature,
...
void RootSubFunctionYSZ::calculate_flux(const PetscScalar* center_node, const PetscScalar* west_node,
...
void RootSubFunctionYSZ::calculate_flux(const PetscScalar* center_node, const int x_index, PetscScalar* flux, const int electron_index, const int ion_index){
...
const double RootSubFunctionYSZ::get_final_face_to_center_width() const{
    return this->x_face[this->nodes] - this->x_center[this->nodes-1];
};


void RootSubFunctionYSZ::calculate_source_term(const PetscScalar* unit, PetscScalar* source, const int electron_index, const int ion_index) const{
    //No source term in YSZ

};

const double RootSubFunctionYSZ::calculate_ion_conductivity(const PetscScalar* unit) const{

    // [Zhu 2008] Modelling distributed charge-transfer proceses in SOFC membrane electrode assemblies
    double sigma = 3.6e7/this->temperature * exp(-80.116e3/this->R/this->temperature);
