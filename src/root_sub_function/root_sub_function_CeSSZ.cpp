...
RootSubFunctionCeSSZ::RootSubFunctionCeSSZ( const int _rank, const std::vector<double>& _x_center, const std::vector<double>& _x_face, const double _temperature,
...
void RootSubFunctionCeSSZ::calculate_flux(const PetscScalar* center_node, const PetscScalar* west_node,
                                        const int x_index, PetscScalar* flux, const int electron_index, const int ion_index){
    
    if(x_index > 0){ //Flux at all cell faces inside CeSSZ
        //Extract values
    ...
    else{ //Flux at the interface between CeSSZ and the preceding layer
        // other|CeSSZ
        //  | * | * |
        //    W w P e
        //      ^
        ...
        //is not a function of mu_o_W (CeSSZ is a dense oxygen anion conductor, a Neumann boundary
...
void RootSubFunctionCeSSZ::calculate_flux(const PetscScalar* center_node, const int x_index, PetscScalar* flux, const int electron_index, const int ion_index){

    //?? Not configured yet
    std::cerr << "Attempt to calculate the flux at the western interface between CeSSZ and the air or fuel compartment - not yet configured" << std::endl;

}; 

const double RootSubFunctionCeSSZ::get_final_face_to_center_width() const{
    return this->x_face[this->nodes] - this->x_center[this->nodes-1];
};


void RootSubFunctionCeSSZ::calculate_source_term(const PetscScalar* unit, PetscScalar* source, const int electron_index, const int ion_index) const{
    //No source term in CeSSZ

};

const double RootSubFunctionCeSSZ::calculate_ion_conductivity(const PetscScalar* unit) const{

    // [Abbas 2011] Preperation and conductivity of ternary scandia stabilised zirconia
    double sigma = 0.003e9/this->temperature * exp(-0.55*this->F/this->R/this->temperature);
