...
RootSubFunctionNiCGO::RootSubFunctionNiCGO( const int _rank, const std::vector<double>& _x_center,const std::vector<double>& _x_face,
...
rank(_rank),x_center(_x_center),x_face(_x_face),galvanostatic(_galvanostatic),applied_I_or_V(_applied_I_or_V){
    ...
    //Identify index for hydrogen and water species
    for(int i = 0; i < this->number_of_gas_species; i++){
        if(this->gas_species[i] == "H2"){
            hydrogen_index = i;
        }
        if(this->gas_species[i] == "H2O"){
            water_index = i;
        }        
    }

    if(hydrogen_index < 0){
        std::cerr << "Warning: hydrogen species not found for Ni-CGO - not compatible with B-V rate (RootSubFunctionNiCGO)" << std::endl;
    }

    if(water_index < 0){
        std::cerr << "Warning: water species not found for Ni-CGO - not compatible with B-V rate (RootSubFunctionNiCGO)" << std::endl;
    }
    ...
    const double S_Ni = this->Phi_Ni/(this->Phi_Ni + ratio_dp*(1 - this->Phi_Ni));
    const double S_CGO = this->Phi_CGO/(this->Phi_CGO + ratio_dp*(1 - this->Phi_CGO));
    this->Percolation_Ni = 1 - pow((4.236 - 6*S_Ni)/(2.472) , 3.7);
    this->Percolation_CGO = 1 - pow((4.236 - 6*S_CGO)/(2.472) , 3.7);
    const double zeta_Ni = this->Phi_Ni/(this->Phi_Ni + pow(ratio_dp,3.0)*(1 - this->Phi_Ni));
    const double zeta_CGO = this->Phi_CGO/(this->Phi_CGO + pow(ratio_dp,3.0)*(1 - this->Phi_CGO));

    const double nr_particles = (1-this->porosity)/(3.1416/6*(zeta_Ni*pow(this->particle_diameter,3)+zeta_CGO*pow(this->particle_diameter,3)));
    const double r_c = this->particle_diameter/2*sin(10*3.1416/180);
    const double ZNi_CGO = 2*(3+(6-3)*pow(this->particle_diameter,2)/((zeta_Ni+zeta_CGO)*pow(this->particle_diameter,2)));
    //const double ZNi_CGO = (3*(2-sqrt(3))*6*(ratio_dp+1))/(1 + ratio_dp - sqrt(ratio_dp*(ratio_dp+2)));

    this->TPB_length = nr_particles*zeta_Ni*zeta_CGO*Percolation_Ni*Percolation_CGO*ZNi_CGO*2*3.1416*r_c;

    //Thermodynamic data retrieved from NIST
    const double dH_H2 = 0 + 33.066178*this->temperature/1000 - 11.3634*pow(this->temperature/1000,2)/2 + 11.4328*pow(this->temperature/1000,3)/3 - 
                        2.772874*pow(this->temperature/1000,4)/4 - 0.158558*pow(this->temperature/1000,-1);
    const double dS_H2 = 130.68;
    const double dH_H2O = -241.83e3 + 30.092*this->temperature/1000 - 6.832514*pow(this->temperature/1000,2)/2 + 6.79345*pow(this->temperature/1000,3)/3 - 
                        2.53448*pow(this->temperature/1000,4)/4 + 0.082139*pow(this->temperature/1000,-1);
    const double dS_H2O = 188.853;
    this->mu_H2_0 = dH_H2 - this->temperature*dS_H2;
    this->mu_H2O_0 = dH_H2O - this->temperature*dS_H2O;

    this->set_effective_binary_diffusion_coefficients_constant_quotient(this->porosity,this->tortuosity);
}


void RootSubFunctionNiCGO::calculate_flux(  const PetscScalar* center_node, const PetscScalar* west_node, const int x_index, PetscScalar* flux,
                                            const int electron_index, const int ion_index){
    if(x_index > 0){ //Flux at all cell faces inside Ni_CGO //?? copied from LSCF?           
        //Extract values
    ...
    else{ //Flux at interface between Ni-CGO and the preceding layer
    //A preceding layer must exist, as this calculate_flux overload is called with west_node input
        ...
        if(this->preceding_layer_is_dense){ //Dense-porous interface
            ...
            if(this->preceding_layer_conducts_electrons){
                std::cerr << "Attempt to calculate electron the flux at the western interface between an electron conducting phase and Ni-CGO - not yet configured" << std::endl;
            }
            else{
            ...
            //This implementation is the same as for the case of oxygen anions in CGO and the preceding layer.
            //Electrons
            const PetscScalar mu_e_W = west_node[electron_index];
            const PetscScalar mu_e_P = center_node[electron_index];
            const double sigma_e_P = this->calculate_electron_conductivity(center_node);
...
void RootSubFunctionNiCGO::calculate_flux(  const PetscScalar* center_node, const int x_index, PetscScalar* flux, const int electron_index,
                                            const int ion_index){
    ...
    if(x_index == 0){ //Interface with air compartment
        //Mass flux
        std::cerr << "Warning: attempting to calculate the flux at the air compartment-Ni-CGO interface - not yet fully configured" << std::endl;
        ...
        std::cerr << "Warning: calculate_flux function overload for the gas compartment interfaces called for invalid x_index. x_index is " << x_index << std::endl;
    }                                                

}; 


const double RootSubFunctionNiCGO::get_final_face_to_center_width() const{
    return this->x_face[this->nodes] - this->x_center[this->nodes-1];
};

void RootSubFunctionNiCGO::calculate_source_term(const PetscScalar* node, PetscScalar* source, const int electron_index, const int ion_index) const{
    ...
    const double p_H2 = node[this->hydrogen_index]*this->R*this->temperature/this->p_atm;
    const double p_H2O = node[this->water_index]*this->R*this->temperature/this->p_atm;
    const double p_H2_eq = this->gas_compartment->inlet_mole_fractions[this->hydrogen_index];
    const double p_H2O_eq = this->gas_compartment->inlet_mole_fractions[this->water_index];
    
    const double eta = (2*mu_e - mu_o + this->mu_H2_0 - this->mu_H2O_0 + this->R*this->temperature*std::log(p_H2/p_H2O))/(2.0*this->F);

    //Kinetic data from Reversible H2/H2O  electrochemical conversion mechanism on the patterned nickel electrodes
    const double i0 = 0.000103038*std::exp(-100886/this->R/this->temperature)*this->TPB_length*2*this->F*std::pow(p_H2,0.0485)*std::pow(p_H2O,0.290);
    
    const double i = i0 * ( std::exp(this->alpha*2*this->F*eta/(this->R*this->temperature)) - 
                            std::exp(-(1.0-this->alpha)*2*this->F*eta/(this->R*this->temperature)) );

    source[this->hydrogen_index] = -i/(2*this->F);
    source[this->water_index] = i/(2*this->F);
    source[electron_index] = i; //Electrons
    source[ion_index] = -i; //Oxygen anions
};


const double RootSubFunctionNiCGO::calculate_electron_conductivity(const PetscScalar* node) const{

    double sigma0 = 3.27e6 - 1065.3*this->temperature;
    double sigma = 1e2*sigma0*pow((1 - this->porosity)*this->Phi_Ni*this->Percolation_Ni, 1.5);
    
    //?? implement no species situation
    return sigma;
};

const double RootSubFunctionNiCGO::calculate_ion_conductivity(const PetscScalar* node) const{
    ...
    const double sigma = 1e2*sigma0*pow(1-this->porosity*this->Phi_CGO*this->Percolation_CGO, 1.5);
