...
class RootSubFunction3YSZ : public RootSubFunction{
private:
    int rank;
    double temperature;
    int nodes;
...
public:
    RootSubFunction3YSZ( const int _rank, const std::vector<double>& _x_center, const std::vector<double>& _x_face, const double _temperature,
                        const int layer_index, const std::shared_ptr<CellConfiguration>& cell_configuration);

    ~RootSubFunction3YSZ() override{};
