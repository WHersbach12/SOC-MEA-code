...
RootSubFunctionNiYSZ::RootSubFunctionNiYSZ( const int _rank, const std::vector<double>& _x_center,const std::vector<double>& _x_face,
...
rank(_rank),x_center(_x_center),x_face(_x_face),galvanostatic(_galvanostatic),applied_I_or_V(_applied_I_or_V){
    ...
    if(hydrogen_index < 0){
        std::cerr << "Warning: hydrogen species not found for Ni-YSZ - not compatible with B-V rate (RootSubFunctionNiYSZ)" << std::endl;
    }

    if(water_index < 0){
        std::cerr << "Warning: water species not found for Ni-YSZ - not compatible with B-V rate (RootSubFunctionNiYSZ)" << std::endl;
    }
    ...
    const double nr_particles = (1-this->porosity)/(3.1416/6*(zeta_Ni*pow(this->particle_diameter,3)+zeta_YSZ*pow(this->particle_diameter,3)));
    const double r_c = this->particle_diameter/2*sin(10*3.1416/180);
    const double ZNi_YSZ = 2*(3+(6-3)*pow(this->particle_diameter,2)/((zeta_Ni+zeta_YSZ)*pow(this->particle_diameter,2)));
    //const double ZNi_YSZ = (3*(2-sqrt(3))*6*(ratio_dp+1))/(1 + ratio_dp - sqrt(ratio_dp*(ratio_dp+2)));

    this->TPB_length = nr_particles*zeta_Ni*zeta_YSZ*Percolation_Ni*Percolation_YSZ*ZNi_YSZ*2*3.1416*r_c;
...
void RootSubFunctionNiYSZ::calculate_flux(  const PetscScalar* center_node, const PetscScalar* west_node, const int x_index, PetscScalar* flux,
                                            const int electron_index, const int ion_index){
    if(x_index > 0){ //Flux at all cell faces inside Ni_YSZ //?? copied from LSCF?           
        //Extract values
    ...
    else{ //Flux at interface between Ni-YSZ and the preceding layer
...
void RootSubFunctionNiYSZ::calculate_flux(  const PetscScalar* center_node, const int x_index, PetscScalar* flux, const int electron_index,
                                            const int ion_index){
    ...
    if(x_index == 0){ //Interface with air compartment
        //Mass flux
        std::cerr << "Warning: attempting to calculate the flux at the air compartment-Ni-YSZ interface - not yet fully configured" << std::endl;
...
const double RootSubFunctionNiYSZ::get_final_face_to_center_width() const{
    return this->x_face[this->nodes] - this->x_center[this->nodes-1];
};

void RootSubFunctionNiYSZ::calculate_source_term(const PetscScalar* node, PetscScalar* source, const int electron_index, const int ion_index) const{
    ...
    const double i0 = 0.00144964*std::exp(-132430/this->R/this->temperature)*this->TPB_length*2*this->F*std::pow(p_H2,0.197994)*std::pow(p_H2O,0.776696);
...
const double RootSubFunctionNiYSZ::calculate_electron_conductivity(const PetscScalar* node) const{

    double sigma0 = 3.27e6 - 1065.3*this->temperature;
    // double sigma = 1e2*sigma0*1999*pow(this->particle_diameter*1e6,3.305)*pow((1 - this->porosity)*this->Phi_Ni*this->Percolation_Ni, 1.5);
    double sigma = 1e2*sigma0*pow((1 - this->porosity)*this->Phi_Ni*this->Percolation_Ni, 1.5);    
    //?? implement no species situation
    return sigma;
};

const double RootSubFunctionNiYSZ::calculate_ion_conductivity(const PetscScalar* node) const{
    
    // [Zhu 2008] Modelling distributed charge-transfer proceses in SOFC membrane electrode assemblies
    // double sigma0 = 3.34e2 * exp(-10300/this->temperature);
    double sigma0 = 3.6e5/this->temperature * exp(-80.116e3/this->R/this->temperature);
    // double sigma = 1e2*sigma0*1999*pow(this->particle_diameter*1e6,3.305)*pow((1 - this->porosity)*this->Phi_YSZ*this->Percolation_YSZ, 1.5);
